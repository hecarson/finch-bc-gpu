#=
Note from Carson:
This is fvad2dgpucode-fix.jl, but with a new kernel for boundary conditions generated by gen_cuda.jl.
Changes are annotated with NEW and CHANGED comments.

Additionally, using the new BC kernel causes the last element in the solution array of u to be 0,
which is incorrect. The differences can be compared using check_sol_error.py.
=#
