#=
Note from Carson:
bte2dgpucode-bcgpu-orig.jl, but modified to use the new boundary condition kernels generated by gen_cuda.jl.
Changes are annotated with NEW and CHANGED comments.

The solutions produced by this solver code are deterministic, as opposed to bte2dgpucode-bcgpu-orig.jl.
bte2dgpu-sol-bcgpu-newkernel-1.txt and bte2dgpu-sol-bcgpu-newkernel-2.txt are two runs of example-bte2d-gpu.jl with
this solver code, and checking the sha256 hash of the two files verifies that the solutions are identical.

However, the error between the new BC kernel solution and the BC on CPU solution (bte2dgpu-sol-bcgpu-newkernel-1.txt
and bte2dgpu-sol.txt) is a little high. Using `check_sol_error.py bte2dgpu-sol.txt bte2dgpu-sol-bcgpu-newkernel-1.txt`
shows that the maximum percent error is 6.35 * 10^-6 percent. The maximum percent with the advection2d example between
the BC on CPU and BC on GPU solutions is 3.62 * 10^-14 percent. The percent errors differ by several orders of
magnitude.
=#
...
function generated_solve_function_for_I(var::Vector{Variable{FT}}, mesh::Grid, refel::Refel, geometric_factors::GeometricFactors, fv_info::FVInfo, config::FinchConfig, coefficients::Vector{Coefficient}, variables::Vector{Variable{FT}}, test_functions::Vector{Coefficient}, ordered_indexers::Vector{Indexer}, prob::FinchProblem, time_stepper::Stepper, buffers::ParallelBuffers, timer_output::TimerOutput, nl_var=nothing) where {FT<:AbstractFloat}
    ...
    @timeit timer_output "GPU alloc" begin
        ...
        # ====================== bdry alloc ===========================
        # CHANGED: nfaces and total_iters are set before calling BC kernels, split mesh.bdryface into individual arrays
        #   by boundary ID
        ...
        # CHANGED: define max_nfaces and max_total_iters for allocating facex_gpu
    ...
    @timeit timer_output "time_steps" begin
        ...
        for ti = 1:time_stepper.Nsteps
            ...
            @timeit timer_output "step_assembly" begin
                ...
                # CHANGED: call new BC kernels instead of Amarnath's BC kernel
