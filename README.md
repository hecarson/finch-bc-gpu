...
`profile`: modified Finch example files from `testing` for profiling with larger mesh sizes
...
Running a Finch script, such as one of the examples, can take a long time (roughly 30 seconds to 2 minutes), which can be very annoying when debugging. Most of this time is spent by the Julia runtime compiling the Julia code. There should be ways to avoid this compilation time cost for future runs, but I have not looked into this.

CUDA profiling of the solve phase of a Finch script, which is when solver codes are executed, seems to break when run in a Slurm interactive job (`salloc`), and runs fine when run as a Slurm batch job (`sbatch`). Batch files are in `batches` to run the Finch scripts under `profile` as Slurm batches.