function ad_bc1_bi_1_gpu(t, max_fi, mesh_bdryface, mesh_face2element, mesh_bids, geometric_factors_volume, geometric_factors_area, dofs_per_node, faceCenters, dim_faceCenters, facex, t, boundary_flux, boundary_dof_index, global_vector)
@inbounds begin
...
function ad_bc2_bi_2_gpu(max_fi, mesh_bdryface, mesh_face2element, mesh_bids, geometric_factors_volume, geometric_factors_area, dofs_per_node, faceCenters, dim_faceCenters, facex, t, boundary_flux, boundary_dof_index, global_vector)
@inbounds begin
thread_id = threadIdx().x + blockDim().x * (blockIdx().x - 1)
if thread_id > max_fi
    return
